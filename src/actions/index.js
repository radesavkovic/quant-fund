...
export const loadInitialPrice = (c20Instance, accounts) => async dispatch => {
  console.log("XXXXXXXXX", c20Instance)

  const currentPrice = await c20Instance.methods.currentPrice().call()
  console.log("YYYYYYYYYY", currentPrice)
  const lastUpdateTime = await c20Instance.previousUpdateTime()
...
export const loadUserBalance = (c20Instance, account) => dispatch => {
  console.log("LOAD USER BALANCE", c20Instance)
  c20Instance.methods.balanceOf(account).call().then(precisionBalance =>
    dispatch({
...
export const getEtherPrice = displayCurrency => dispatch => {
  const priceObj = {"prices":[{"currency":"USD","price":2592.380000000000000000},{"currency":"BTC","price":0.06823686724978876520429892422}],"status":"success"};
